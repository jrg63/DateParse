...
DateParse(dp_str, americanOrder := 0) 
{
	...
	{ 	; NOT timestring only eg 1535
		...
		else 
		{
			...
			if (RegExMatch(day, monthNames) or americanOrder and !RegExMatch(month, monthNames) or (month > 12 and day <= 12))
			{ ;try to infer day/month order
	...
	if (day or month or year) and not (day and month and year) 
	{ ; partial date
		if not month or not (day or month) or (hour and not day)
		{ ; partial date must have month and day with time or day or year without time
			return
		}
		else if not day 
		{ ; without time use 1st for day if not present
			day := 1
		}
	}
